...
You aren't limited to specific, hardcoded routes. Arachne (via Pedestal) also allows you to use a Clojure keyword in a route name to create a _path parameter_. Values at that path are then available in the request, in a map under the `:path-params` key.
